...
$(document).ready(function() {
    "use strict";
    $("#resultlist").sortable({
        cancel: ".contenteditable,.deleteButton"
    });
    ...
    var reHTTP = new RegExp("h.*://"),
        ...
        opacity = 0.15,
        colours = ["RGBA(160, 0, 46, " + opacity + ")",
            "RGBA(231, 163, 58, " + opacity + ")",
            "RGBA(244, 202, 88, " + opacity + ")",
            "RGBA(167, 194, 68, " + opacity + ")",
            "RGBA(68, 131, 146, " + opacity + ")",
            "RGBA(158, 184, 254, " + opacity + ")",
            "RGBA(231, 154, 255, " + opacity + ")",
            "RGBA(186, 94, 232, " + opacity + ")", "RGBA(67, 40, 187, " +
            opacity + ")"
        ],
        ...
        resultClicked = false;
    if (date - 1 === 0) {
        yesterday = monthNames[parseInt(now.getMonth(), 10) - 1] + " " +
            dayNumbers[parseInt(now.getMonth(), 10)];
    }
    ...
    $('#resultlist').on('click', '.toggle', function() {
        $(this).parent('li').find('.sectiontitle').toggleClass(
            'hidden');
        $(this).parent('li').find('.related').toggleClass(
            'hidden');
        $(this).parent('li').find('ul.context').toggleClass(
            'hidden');
    ...
    $('#go').click(function() {
        if (clicked === true) {
            var r = window.confirm(
                "This will erase what you've already done!\n\nAre you sure?"
            );
            if (r === false) {
                return false;
            }
        ...
        clicked = true;
        $('#copy').prop('disabled', false).removeClass(
            'disabled');
        //inits
        $('#resultlist').html('');
        ...
        titlearray = [];
        completedarray = [];
        $('.header').append(monthNames[now.getMonth()] + " " +
            now.getDate() + ", " + now.getFullYear());
        $('.description').append(
            "In today's report...<br/><br/>");
        ...
        $(urls).each(function() {
            var fetchURL = this,
                ...
                websiteName = newURL.replace(website,
                    "").replace(trim, ""),
                e = 0,
                postdata = {
                    action: daction,
        ...
        p.then(function() { // when all the actions are done
            ...
            $(".counter").html(
                ...
                completedarray[u] + "</span> Item "
            );
            $('input[name="rawurl"]').val(urlarray[0]);
            $('input[name="publication"]').val(pubarray[
                0]);
            $('input[name="pagetitle"]').val(titlearray[
                0]);
    ...
    $('#newTopic').click(function() {
        ...
        $($('<option>', {
            ...
            text: newTopic
        })).appendTo('#subject').attr('selected',
            'selected');
    ...
    $('#pagination').change(function() {
        var newPage = $("#pagination option:selected").text() -
            1;
        u = newPage;
        ...
        $(".counter").html("<span class='completed'>" +
            completedarray[newPage] + "</span> Item ");
    ...
    $('#addNew').click(function() {
        ...
        var newURL = $('input[name="rawurl"]').val(),
            ...
            newSubject = $('select[name="subject"]').children(
                "option").filter(":selected").val(),
            newSubjectText = $('select[name="subject"]').children(
        ...
        if ($(".date" + momentDate)[0]) { //If there is a date already
            if ($(ifStatement)[0]) {
                $(".date" + momentDate).next(ifStatement).last()
                    .after("<li class='meta " + newSubject +
                        "' style='background:" + colours[random] +
                        "'><p class='deleteButton'>×</p><span class='related'>Related Stories:<br/></span><span class='entry contenteditable' contenteditable='true'>" +
                        newPublication + " - " + newTitle +
                        "<br/></span>" + newURL +
                        "<a class='toggle'> &bull; toggle</a><br /></li>"
                    );
            } else {
                $('.date' + momentDate).after(
                    "<li class='meta " + newSubject +
                    "' style='background:" + colours[random] +
                    "'><p class='deleteButton'>×</p><span class='sectiontitle contenteditable' contenteditable='true'>" +
                    newSubjectText +
                    "<br/></span><span class='related hidden'>Related Stories:<br/></span><span class='entry contenteditable' contenteditable='true'>" +
                    newPublication + " - " + newTitle +
                    "<br/></span>" + newURL +
                    "<a class='toggle'> &bull; toggle</a><br /><br /><ul class='context'><li class='contenteditable quote' contenteditable='true'>" +
                    newContext +
                    "<br/></li><br/></ul></li>");
            }
            ...
            if ($('.date')[0]) {
                ...
                $('.resultlist').append("<li class='date date" +
                    momentDate + "' id='" + momentDate +
                    "'><p>" + newDate + "</p></li>").append(
                    "<li class='meta " + newSubject +
                    "' style='background:" + colours[random] +
                    "'><p class='deleteButton'>×</p><span class='sectiontitle contenteditable' contenteditable='true'>" +
                    newSubjectText +
                ...
                $('.resultlist').prepend("<li class='date date" +
                    momentDate + "' id='" + momentDate +
                    "'><p>" + newDate + "</p></li>").append(
                    "<li class='meta " + newSubject +
                    "' style='background:" + colours[random] +
        ...
        var firstComplete = !!completedarray.reduce(function(a,
            b) {
            return (a === b) ? a : NaN;
        }); // From http://stackoverflow.com/a/21266395
        if (firstComplete === true) {
            $(".counter").html(
                "<span class='finished'>It looks like all URLs have been processed!</span><br/><span class='finished'>Hit \"copy to clipboard\" to finish.</span><br/><span class='completed'>" +
                completedarray[u] + "</span> Item ");
        }
    });
